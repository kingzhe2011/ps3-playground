...
*/
function do_read(addr, len)
{
    try {
        if (len <= 30000) {
            var bytes = get_bytes(addr, len);
            sendcmsg("read", addr, bytes);
        } else {
            var n = len;
            var xaddr = addr;
            while(n > 30000) {
                var bytes = get_bytes(xaddr, 30000);
                sendcmsg("read", xaddr, bytes);
                n -= 30000;
                xaddr += 30000;
            }
            var bytes = get_bytes(xaddr, n);
            sendcmsg("read", xaddr, bytes);
        }
    } catch(e) {
        logdbg("ReadError: " + e);
...
*/
function do_dump(addr, len, fname)
{
    try {
        if (len <= 30000) {
            var bytes = get_bytes(addr, len);
...
*/
function do_search(begaddr, endaddr, pattern)
{
    try {
        var score = 0;
        var found = -1;
...
*/
function do_search_hex(begaddr, endaddr, pattern)
{
    try {
        var score = 0;
        var found = -1;
...
*/
function scan_back(begaddr, step)
{
    var curr = begaddr;
    while(true) {
        var bytes = get_bytes(curr, step);
...
*/
function scan_mod_info(addr)
{
    try {
        var searchaddr = addr;
        while(true) {
            var found = do_search(searchaddr, 0xffffffff, MOD_SIG);
            ...
            // Get mod_info struct
            var maddr = found - 4;
            var buf = get_bytes(maddr, MODINFO_SIZE);
...
*/ 
function disp_minfo(addr)
{
    try {
        var buf = get_bytes(buf, MODINFO_SIZE);
...
*/ 
function disp_imp(addr, n)
{
    try {
        var maddr = addr;
        for (var i = 0; i < n; i++) {
            var buf = get_bytes(maddr, MODIM_SIZE);
            var libptr = buf.substring(40, 48);
            libptr = parseInt(libptr, 16);
            libptr = swap32(libptr);
            var libname = "2222";
            if (libptr != 0) {
                libptr_e = do_search(libptr, 0xffffffff, "\x00");
                libname = get_bytes(libptr, libptr_e - libptr);
            }
...
*/ 
function disp_ex(addr, n)
{
    try {
        var maddr = addr;
        for (var i = 0; i < n; i++) {
            var buf = get_bytes(maddr, MODX_SIZE);
            var libptr = buf.substring(40, 48);
            libptr = parseInt(libptr, 16);
            libptr = swap32(libptr);
            var libname = "2222";
    ...
    try {
        while (true) {
            ...
            if(cmd_s[0] == "exit") {
                ...
                var pattern = cmd_s[3];
                do_search(begaddr, endaddr, pattern);
            }
            else if(cmd_s[0] == 'sh'){
                if (cmd_s.length < 3) {
                ...
                var endaddr = Number(cmd_s[2]);
                var pattern = cmd_s[3];
                do_search_hex(begaddr, endaddr, pattern);
            }
